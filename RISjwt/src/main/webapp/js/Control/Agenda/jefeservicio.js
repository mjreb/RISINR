...
$(document).ready(function () {
    ...
    switch(PERFIL[0]["rol"]){
        ...
        case 'JefedelServicio':
            ...
            //Se llenan las opciones de los formularios de estudio y sala
            //consultaestudios(PERFIL[0]["idarea"]);
            consultaestudiosDVG(PERFIL[0]["idarea"]); //CAmbio por DVG
...
//Función para remplazar la anterior
function consultaestudiosDVG(idarea){
    var jsonconsulta = new Object();
    jsonconsulta.idarea = idarea;
    RESTObtenerListBoxDVG( uriserv + "/getStudiesDVG", jsonconsulta, "POST");
}
...
function CreateTableFromREST(uri, jsondata, nameDivContainer, tablName, col, colsort, noedit){
    ...
    console.log("tabla"+tablName);
    console.log("DIV"+nameDivContainer);    
    //Metodo AJAX para realizar peticiones al servicio REST.
    $.ajax({
        //Objetos configurables para realizar peticiones al servicio REST.
        url: uri, 
        ...
        if(data[0].length!==0){
            ...
            console.log("Error metodo CreateTableFromREST, linea 1484");
...
function UpdateTableFromJson(jsondata, nameDivContainer, tablName, col, colsort, noedit){
    console.log(jsondata);
    console.log(tablName);
    console.log(nameDivContainer);
 ...
 * Función para dar formato a la tabla.
  * @argument {String} tablName
 * @argument {String} coledit
 * @argument {String} coldelete 
 * @argument {String} noedit
 */
 ...
 * @argument {String} uri 
 * @argument {String} identificador
 * @argument {String} texto
 * @argument {String} valor
 * @argument {String} jsondata
 * @argument {String} metodo
 */
//MODIFICADA POR DVG: 04082022
    ...
    $.ajax({
        ...
        if(data[0].length!==0){
           ...
           if(data.length>12){
                ...
                console.log(data);
                var equipos=JSON.parse(data[0]);
                var usuarios=JSON.parse(data[1]);
                var estudios=JSON.parse(data[2]);
...
//MODIFICADA POR DVG: 04082022
function RESTObtenerListBoxDVG(uri,jsondata, metodo){
    console.log(uri);
    //Metodo AJAX para realizar peticiones al servicio REST.
    $.ajax({
        //Objetos configurables para realizar peticiones al servicio REST.
        ...
        if(data[0].length!==0){
            ...
            console.log(usuarios.Usuarios);
            console.log(estudios.Estudios);
            updatelistbox("consultaestudio", "stdyName", "stdyId", estudios.Estudios);
            updatelistbox("consultasala", "sala", "sala", equipos.Equipos);
        }else{
           //alert(data[0]["salida"]);
           alert("Datos: "+data[0]["salida"]);
           cleanlistbox("consultaestudio"); 
           cleanlistbox("consultasala");
        }
    }).fail(function (jqXHR, textStatus, errorThrown) {
        alert("No hay datos en bd " + errorThrown);
    }).always(function (jqXHROrData, textStatus, jqXHROrErrorThrown) {
 ...
 * @argument {String} identificador 
 * @argument {String} texto 
 * @argument {Integer} valor
 * @argument {Array} jsonarray
 */
