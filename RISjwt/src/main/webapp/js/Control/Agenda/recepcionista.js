...
function consultaagenda(){
    ...
    console.log(jsonconsulta.nombrepaciente);
    console.log(jsonconsulta.apellidopaternopaciente);
    console.log(jsonconsulta.apellidomaternopaciente);
    
    /*
    for (let i = 0; i < jsonconsulta.nombrepaciente.length; i++) {
        let codigoChar = jsonconsulta.nombrepaciente.charCodeAt(i); // Obtiene el código Unicode del carácter en la posición i
        if (codigoChar < 32 || codigoChar === 127) { // Rango de caracteres no imprimibles
            console.log("Carácter no imprimible encontrado en la posición " + i + ": '" + str.charAt(i) + "' (Código Unicode: " + codigoChar + ")");
            return true; // Si se encuentra un carácter no imprimible, devuelve true
        }
    }
    console.log("No se encontraron caracteres imprimibles");
    */
    ...
    var llavesRSAconsulta = KEYUTIL.generateKeypair("RSA", 2048);// Generar el par de llaves RSA
    var llavePrivadaconsulta = KEYUTIL.getPEM(llavesRSAconsulta.prvKeyObj,"PKCS8PRV");//Obtener la llave privada RSA
    var llavePublicaconsulta = KEYUTIL.getPEM(llavesRSAconsulta.pubKeyObj);//Obtener la llave publica RSA
    var header ={ alg: "RS256", typ: "JWT"};//cabecera de JWT
    var JSONToken = KJUR.jws.JWS.sign("RS256", JSON.stringify(header), JSON.stringify(jsonconsulta), llavePrivadaconsulta);//Creación del JWT
    var respuestaFrontend = {"llavepublica" : llavePublicaconsulta,"token" : JSONToken};//Contenedor para la llave publica y el JWT
    
    console.log(llavePublicaconsulta);// imprimir en consola la llave pública
    console.log(JSONToken);// imprimir en consola el JWT
    ...
    if(PERFIL[0]["rol"] === 'SubdirectordeServicios'){
        ...
        $.ajax({
            //Objetos configurables para realizar peticiones al servicio REST.
            url: uriserv + "/getAppointment", 
            type: 'POST',
            dataType:'json',
            data: respuestaFrontend
        }).done(function (data, textStatus, jqXHR){
            ...
            if (isValid) {
                ...
                var objetojson = JSON.parse(subject);
                UpdateTableFromJson(objetojson, "showDataAgendar", "tblagenda", ["Estudio","Fecha","Paciente","Turno","Sala","Horario","Estado"], ["estudio","fechacita","paciente","turno","ubicacion","horacita","estado"],"0");
            }    
...
function DialogoBusqueda(e){
    var val=e.target.id;
    switch(val){
        case 'consultaCita':
            document.querySelector(".modalBuscaAgenda").style.display = "block"; 
            break;
        case 'cerrarDialogCita':
            document.querySelector(".modalBuscaAgenda").style.display = "none"; 
           break;
    }
